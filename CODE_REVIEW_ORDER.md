...
40. `testdata/protocol-parsing-policy.json`
    - Shared strict base64url, RFC3339 timestamp, and canonical MIME parsing fixtures.

41. `internal/cryptoenv/reference_test.go`
    - Reference round-trip, negative vector rejection, manifest validation, and sanitization tests.

42. `docs/protocol-reference.md`
    - Protocol implementation pointers and deterministic positive/negative test-vector documentation.

43. `web/drop-page/index.html`
    - Sender-facing mobile-friendly drop page shell and canonical UI copy.

44. `web/drop-page/styles.css`
    - Sender-facing drop page styles.

45. `web/drop-page/app.js`
    - Browser WebCrypto encryption, native-picker Camera mode, immutable retries, page-local sent history, and reusable submission flow.
    - `web/drop-page/app.test.cjs`: Node built-in tests for repeated sends, capture auto-send/cancellation, pending-photo safety, retries, expiry, and history. Run with `node --test web/drop-page/app.test.cjs`.

46. `web/drop-page/assets.go`
    - Embedded static asset filesystem for the drop page.

47. `internal/httpapi/health.go`
    - Low-information `/health` handler.

48. `internal/httpapi/responses.go`
    - Shared JSON response and error helpers for API handlers.

49. `internal/httpapi/auth.go`
    - Bearer API token parsing and SQLite token-hash authentication.

50. `internal/httpapi/create.go`
